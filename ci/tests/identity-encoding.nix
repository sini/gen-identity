...
in
{
  ...
  # ★ THE STRADDLING CELL, RELOCATED. This asserted the mint's FORMAT — the kind joined to a
  # digest of the pairs — from `identity-hash-for.nix`, whose subject is the REFLECTION path
  # that decides which of a kind's options are identity keys. Its subject was never the
  # reflection path; it is the mint, so it travels with the mint rather than staying behind to
  # document a library that no longer contains its subject. Its format claim is corroborated by
  # four cells below (`:426`, `:432`, `:436`, `:452` in the file it came from), so neither file
  # loses coverage in the move.
  flake.tests.identity-encoding.test-hashIdentity-shape = {
    expr =
